 ...
 * window-like. For TopSheetDialog use {@link BottomSheetDialog#setTitle(int)}, and for
 * TopSheetDialogFragment use {@link ViewCompat#setAccessibilityPaneTitle(View, CharSequence)}.
 */
public class TopSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public abstract static class TopSheetCallback {
         ...
         *
         * @param topSheet The bottom sheet view.
    ...
    // TODO: fix drag behavior and slide animation

    public TopSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);

        peekHeightGestureInsetBuffer = context.getResources().getDimensionPixelSize(R.dimen.mtrl_min_touch_target_size);
        ...
        if (hasBackgroundTint) {
            ColorStateList topSheetColor = getColorStateList(
                    context, a, R.styleable.BottomSheetBehavior_Layout_backgroundTint);
            createMaterialShapeDrawable(context, attrs, hasBackgroundTint, topSheetColor);
        } else {
...
//    }
    ...
    /**
     * Determines the height of the TopSheet in the {@link #STATE_HALF_EXPANDED} state. The
     * material guidelines recommended a value of 0.5, which results in the sheet filling half of the
     * parent. The height of the TopSheet will be smaller as this ratio is decreased and taller as
     ...
     * @attr ref
     * com.google.android.material.R.styleable#TopSheetBehavior_Layout_behavior_halfExpandedRatio
     */
    ...
    /**
     * Gets the ratio for the height of the TopSheet in the {@link #STATE_HALF_EXPANDED} state.
     *
     * @attr ref
     * com.google.android.material.R.styleable#TopSheetBehavior_Layout_behavior_halfExpandedRatio
     */
    ...
    /**
     * Determines the top offset of the TopSheet in the {@link #STATE_EXPANDED} state when
     ...
     * @attr ref
     * com.google.android.material.R.styleable#TopSheetBehavior_Layout_behavior_expandedOffset
     */
     ...
     * @attr ref
     * com.google.android.material.R.styleable#TopSheetBehavior_Layout_behavior_skipCollapsed
     */
     ...
     * @attr ref com.google.android.material.R.styleable#TopSheetBehavior_Layout_behavior_draggable
     */
    ...
    /**
     * Sets save flags to be preserved in topsheet on configuration change.
     *
    ...
    public void setTopSheetCallback(TopSheetCallback callback) {
        Log.w(TAG, "TopSheetBehavior now supports multiple callbacks. `setTopSheetCallback()` removes"
    ...
    void setStateInternal(@State int state) {
        ...
        for (int i = 0; i < callbacks.size(); i++) {
            callbacks.get(i).onStateChanged(topSheet, state);
        }
        updateAccessibilityActions();
    }
    ...
    private void calculateCollapsedOffset() {
        ...
        if (fitToContents) {
            ...
            collapsedOffset = min(parentHeight + peek, -fitToContentsOffset);
        } else {
    ...
    private void createMaterialShapeDrawable(
            ...
            @Nullable ColorStateList topSheetColor) {
        if (this.shapeThemingEnabled) {
            ...
            if (hasBackgroundTint && topSheetColor != null) {
                materialShapeDrawable.setFillColor(topSheetColor);
            } else {
    ...
    void dispatchOnSlide(int top) {
        View topSheet = viewRef.get();
        if (topSheet != null && !callbacks.isEmpty()) {
            float slideOffset =
            ...
            for (int i = 0; i < callbacks.size(); i++) {
                callbacks.get(i).onSlide(topSheet, slideOffset);
            }
        }
    }
    ...
    /**
     * Sets whether the TopSheet should update the accessibility status of its {@link
